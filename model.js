...
app.get('/consultaClienteJuridico', function (req, res) {
	canal.connect(function (erro, conexao, finalizado) {
		...
		var sql = '\nSELECT ID_CLIENTE, NOME_FANTASIA, RAZAO_SOCIAL, CNPJ, EMAIL, TELEFONE, NUMERO, BAIRRO, CEP, CIDADE, ESTADO, ID_EMPRESA '
...
app.post('/atualizaOS', function (req, res){
	...
	canal.connect(function(erro, conexao, feito){
		...
		for (var i in req.body.produto)
			produtos += req.body.produto[i].toString() + ',';
		produtos = produtos.substring(0, produtos.length-1);

		for (var j in req.body.servico)
			servicos += req.body.servico[j].toString() + ',';
		...
		// insere relações que não existentes
			// PRODUTOS
			sql +='\n'
				+ '\nINSERT INTO TB_OS_PRODUTO (id_ordem_servico,id_produto) '
			...
			for (produto in req.body.produto){
				sql +='\n       SELECT ' + req.body.id_ordem_servico + ' AS id_ordem_servico, ' + req.body.produto[produto] + ' AS id_produto';
				if (produto < req.body.produto.length-1)
					sql +='\n        UNION ALL';
			}
			// SELECT /*id_ordem_servico*/ AS id_ordem_servico, /*id_produto*/ AS id_produto
			//	UNION ALL /*quando não for o ultimo*/
			sql +='\n) TB_TEMP_OS_PRODUTO'
			...
			for (servico in req.body.servico){
				sql +='\n       SELECT ' + req.body.id_ordem_servico + ' AS id_ordem_servico, ' + req.body.servico[servico] + ' AS id_servico';
				if (servico < req.body.servico.length-1)
					sql +='\n   	 UNION ALL';
			}
			// SELECT /*id_ordem_servico*/ AS id_ordem_servico, /*id_servico*/ AS id_servico
			//	UNION ALL /*quando não for o ultimo*/
			sql +='\n) TB_TEMP_OS_SERVICO '
